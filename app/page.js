...
var TEMA_COLORS = ['#DC2626','#2563EB','#F59E0B','#059669','#7C3AED','#EC4899','#0891B2','#EA580C']

function sc(s) { return STATUS_COLORS[s] || STATUS_COLORS.normal }
function temaColor(i) { return TEMA_COLORS[(i - 1) % TEMA_COLORS.length] }
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded text-white" style={{backgroundColor:color}}>Téma</span>
                <span className="text-xs px-2 py-0.5 rounded" style={{backgroundColor:sc(tema.status).bg,color:sc(tema.status).text}}>{sc(tema.status).label}</span>
              </div>
              <h2 className="text-xl font-semibold text-stone-800">{tema.name}</h2>
            </div>
            <button onClick={onClose} className="text-stone-400 hover:text-stone-600 text-xl p-1">✕</button>
          </div>
        </div>
        <div className="p-6 space-y-5">
          {tema.description && <div className="text-sm text-stone-600">{tema.description}</div>}
          {tema.ai_summary && (<div className="bg-stone-50 border border-stone-100 rounded-xl p-4"><span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">✦ Claude zhrnutie</span><div className="text-sm text-stone-600 mt-1 whitespace-pre-line">{tema.ai_summary}</div></div>)}
          {tema.user_notes && (<div className="bg-amber-50 border border-amber-100 rounded-xl p-4"><span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Moje poznámky</span><div className="text-sm text-amber-800 mt-1 whitespace-pre-line">{tema.user_notes}</div></div>)}

          {relKauzy.length > 0 ? (
            <div><span className="text-stone-400 text-xs uppercase tracking-wider font-bold">Kauzy ({relKauzy.length})</span>
            <div className="mt-2 space-y-2">{relKauzy.map(function(k){var st=sc(k.priority||k.status);return(
              <div key={k.id} className="bg-blue-50 rounded-xl p-3 border border-blue-100">
                <div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold px-2 py-0.5 rounded bg-blue-200 text-blue-800">{k.code}</span><span className="text-xs px-2 py-0.5 rounded" style={{backgroundColor:st.bg,color:st.text}}>{st.label}</span></div>
                <div className="text-sm font-medium text-stone-800">{k.name}</div>
                {k.ai_summary && <div className="text-[11px] text-stone-500 mt-1">{k.ai_summary}</div>}
              </div>)})}</div></div>
...
// --- KONANIE DETAIL ---
function KonanieDetail({ item, onClose, tasks }) {
  var st = sc(item.status)
  var itemTasks = tasks.filter(function(t){return t.konanie_id===item.id})
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto" onClick={function(e){e.stopPropagation()}}>
        <div className="p-6 border-b border-stone-100">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2"><span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-red-100 text-red-800">Konanie</span><span className="text-sm font-mono font-bold px-2 py-0.5 rounded" style={{backgroundColor:st.bg,color:st.text}}>{item.code}</span></div>
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            {item.court && <div><span className="text-stone-400 text-xs uppercase tracking-wider">Súd / Úrad</span><div className="font-medium text-stone-700 mt-0.5">{item.court}</div></div>}
          ...
          {item.ai_summary && (<div className="bg-stone-50 border border-stone-100 rounded-xl p-4"><span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">✦ Claude zhrnutie</span><div className="text-sm text-stone-600 mt-1 whitespace-pre-line">{item.ai_summary}</div></div>)}
          {item.user_notes && (<div className="bg-amber-50 border border-amber-100 rounded-xl p-4"><span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Moje poznámky</span><div className="text-sm text-amber-800 mt-1 whitespace-pre-line">{item.user_notes}</div></div>)}
          {item.next_action && (<div className="bg-amber-50 border border-amber-100 rounded-xl p-3"><span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Ďalší krok</span><div className="text-sm text-amber-800 mt-0.5">{item.next_action}</div></div>)}
          {itemTasks.length > 0 && (<div><span className="text-stone-400 text-xs uppercase tracking-wider">Úlohy ({itemTasks.length})</span><div className="mt-2 space-y-1">{itemTasks.map(function(t){var ts=sc(t.priority);return(<div key={t.id} className="flex items-center gap-2 text-sm py-1"><span className="w-2 h-2 rounded-full" style={{backgroundColor:ts.text}} /><span className="text-stone-700">{t.title}</span></div>)})}</div></div>)}
        </div>
      </div>
    </div>
  )
...
function Dashboard({ user }) {
  ...
  var loadData = useCallback(async function() {
    var [emailsR,docsR,temyR,kauzyR,konaniaR,aktivityR,tasksR,strategiaR,tkR,tkoR] = await Promise.all([
      ...
      supabase.from('aktivity').select('*').order('id'),
  ...
  if (loading) return (<main className="min-h-screen flex items-center justify-center bg-stone-50"><div className="text-center"><div className="text-3xl font-extralight tracking-wide text-stone-300 animate-pulse">OPUS</div></div></main>)
  ...
  var activeTemy = data.temy.filter(function(t){return t.status==='active'})
    ...
    <main className="min-h-screen bg-stone-50">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="hidden md:flex items-center gap-4 pl-6 border-l border-stone-100">
              <div className="text-center"><div className="text-lg font-light text-stone-700">{activeTemy.length}</div><div className="text-[9px] text-stone-400 uppercase tracking-wider">Tém</div></div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-white rounded-2xl border border-stone-100 p-5 shadow-sm"><div className="text-3xl font-light tracking-tight" style={{color:'#6366f1'}}>{activeTemy.length}</div><div className="text-[11px] text-stone-400 mt-1 uppercase tracking-wider font-medium">Aktívne témy</div></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
              <div className="px-5 md:px-6 py-4 border-b border-stone-50 flex justify-between items-center"><h2 className="text-sm font-semibold text-indigo-800 uppercase tracking-wider">Témy</h2><span className="text-[10px] text-stone-400">{activeTemy.length} aktívnych</span></div>
              <div className="p-3 md:p-4 grid gap-3 md:grid-cols-2">
                {data.temy.map(function(tema){
                  ...
                  return (
                    <div key={tema.id} onClick={function(){setSelectedTema(tema)}} className="bg-white rounded-xl border border-stone-100 p-4 hover:shadow-md hover:border-stone-200 cursor-pointer group relative overflow-hidden">
                      ...
                      <div className="pl-3">
                        <div className="flex items-center gap-2 mb-1.5"><span className="text-xs px-2 py-0.5 rounded" style={{backgroundColor:sc(tema.status).bg,color:sc(tema.status).text}}>{sc(tema.status).label}</span></div>
            ...
            <section className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
              ...
              <div className="p-3 md:p-4 grid gap-3">
                {data.konania.map(function(k){var st=sc(k.status);return(
                  <div key={k.id} onClick={function(){setSelectedKonanie(k)}} className="bg-white rounded-xl border border-stone-100 p-4 hover:shadow-md hover:border-stone-200 cursor-pointer group">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2"><span className="text-xs font-mono font-bold px-2 py-0.5 rounded bg-red-100 text-red-800">{k.code}</span><span className="text-xs px-2 py-0.5 rounded" style={{backgroundColor:st.bg,color:st.text}}>{st.label}</span><span className="text-[10px] text-stone-400">{k.our_role}</span></div>
            ...
            {data.aktivity.length > 0 && (
              <section className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
                <div className="px-5 md:px-6 py-4 border-b border-stone-50"><h2 className="text-sm font-semibold text-indigo-800 uppercase tracking-wider">Aktivity</h2></div>
                <div className="p-3 md:p-4 grid gap-3">{data.aktivity.map(function(a){return(<div key={a.id} className="bg-white rounded-xl border border-stone-100 p-4"><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold px-2 py-0.5 rounded bg-indigo-100 text-indigo-800">Aktivita</span><span className="text-xs px-2 py-0.5 rounded" style={{backgroundColor:sc(a.status).bg,color:sc(a.status).text}}>{sc(a.status).label}</span></div><h3 className="text-sm font-semibold text-stone-800">{a.name}</h3>{a.description && <div className="text-[11px] text-stone-400 mt-0.5 truncate">{a.description}</div>}</div>)})}</div>
              </section>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-stone-100/50 rounded-2xl p-4 text-center space-y-1">
              ...
              <div className="text-[11px] text-stone-500">{data.temy.length} tém &bull; {data.kauzy.length} kauz &bull; {data.konania.length} konaní &bull; {data.tasks.length} úloh</div>
              <div className="text-[10px] text-stone-300 mt-2">OPUS v0.6 &mdash; {new Date().getFullYear()}</div>
            </div>
          </div>
        </div>
      </div>
