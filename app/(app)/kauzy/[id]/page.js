...
import { supabase, sc, formatDate, formatDateTime } from '../../../components/opus'
      ...
      <div onClick={function() { setOpen(!open) }} className="p-4 cursor-pointer hover:bg-stone-50 flex items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-[11px] text-stone-400">
            ...
            {email.has_attachments && <span className="text-stone-500">📎 {email.attachment_count}</span>}
          </div>
        </div>
...
export default function KauzaDetailPage() {
  ...
  async function loadLinkedEmails(kid, page) {
    ...
    var { data, count } = await supabase.from('emails')
      ...
      .in('id', ids).order('date', { ascending: false }).range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1)
    setEmails(data || []); setEmailCount(count || 0); setEmailPage(page)
  }
  ...
  async function loadLinkedDocs(kid, page) {
    ...
    var { data, count } = await supabase.from('documents')
      ...
      .in('id', ids).order('date', { ascending: false, nullsFirst: false }).range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1)
    setDocuments(data || []); setDocCount(count || 0); setDocPage(page)
  }

  async function searchEmails(keywords, page) {
    var { data } = await supabase.rpc('search_emails_by_keywords', { keywords: keywords, min_matches: keywords.length > 3 ? 2 : 1, result_limit: PAGE_SIZE, result_offset: page * PAGE_SIZE })
    setEmails(data || []); setEmailCount(data && data.length > 0 ? Number(data[0].total_count) : 0); setEmailPage(page)
  }

  async function searchDocs(keywords, page) {
    var { data } = await supabase.rpc('search_docs_by_keywords', { keywords: keywords, min_matches: keywords.length > 3 ? 2 : 1, result_limit: PAGE_SIZE, result_offset: page * PAGE_SIZE })
    setDocuments(data || []); setDocCount(data && data.length > 0 ? Number(data[0].total_count) : 0); setDocPage(page)
  }
  ...
  if (loading) return <div className="text-center py-20 text-stone-300 text-lg animate-pulse">Načítavam kauzu...</div>

  if (!kauza) return (
    <div className="text-center py-20">
      <div className="text-stone-400 mb-4">Kauza nenájdená</div>
      <Link href="/kauzy" className="text-sm text-indigo-600 hover:underline">← Späť na kauzy</Link>
    </div>
  )
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center gap-2 text-[11px] text-stone-400">
        <Link href="/kauzy" className="hover:text-stone-600">Kauzy</Link>
        <span>/</span>
        <span className="text-stone-600">{kauza.name}</span>
      </div>

      {/* KAUZA INFO */}
      <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          {kauza.code && <span className="text-[10px] font-mono font-bold text-stone-400">{kauza.code}</span>}
        ...
        {konania.length > 0 && (
          <div className="border-t border-stone-100 pt-4 mt-4">
            <div className="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-2">Prepojené konania ({konania.length})</div>
            <div className="space-y-2">
              {konania.map(function(k) {
                var ks = sc(k.status)
                return (
      ...
      {tab === 'emails' && (
        <div className="space-y-3">
          {emails.length === 0 && <div className="text-center py-8 text-stone-300 text-sm">Žiadne emaily — kauza ešte nie je nakurovaná</div>}
      ...
      {tab === 'docs' && (
        <div className="space-y-3">
          ...
          {totalDocPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <button disabled={docPage === 0} onClick={function() { loadLinkedDocs(kauzaId, docPage - 1) }}
                className="px-3 py-1.5 text-sm rounded-lg border border-stone-200 disabled:opacity-30 hover:bg-stone-50">← Novšie</button>
              <span className="text-[11px] text-stone-400">{docPage + 1} / {totalDocPages}</span>
              <button disabled={docPage >= totalDocPages - 1} onClick={function() { loadLinkedDocs(kauzaId, docPage + 1) }}
                className="px-3 py-1.5 text-sm rounded-lg border border-stone-200 disabled:opacity-30 hover:bg-stone-50">Staršie →</button>
            </div>
          )}
        </div>
